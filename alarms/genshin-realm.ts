...
class GenshinRealm extends PersonalAlarm {
    constructor() {
        super("Genshin Realm Currency", 2*24*60*60);
    }
    ...
    async executeUser(userAlarm: UserAlarm) {
        ...
        const newNext = bc.realm_currency.recovery_time - userAlarm.time_before;
        if (newNext > cur) {
            this.waitNext(userAlarm, bc.realm_currency.recovery_time);
        } else {
            this.triggerAlarm(userAlarm, cur + this.waitOnTrigger );
            if (!userAlarm.triggered) {
                const message = await genshin.buildUserEmbed();
                message.content = `Realm currency capped <t:${bc.realm_currency.recovery_time}:R>.`;
                DiscordHelper.whisper(userAlarm.user_id, message);
            } else {
                DiscordHelper.sendToLog("Realm currency is triggered again");
            }
        }
    }
}

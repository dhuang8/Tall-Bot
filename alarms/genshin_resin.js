...
class GenshinResin extends Alarm {
    constructor(client) {
        super(client, "Genshin Resin");
        this.trigger_wait = 10*60*60;
    }
    ...
    async execute(user_alarm) {
        ...
        try {
            ...
            const new_next = bc.resin.recovery_time - user_alarm.time_before;
            if (new_next > cur) {
                ...
                if (!user_alarm.triggered) {
                    this.client.users.fetch(user_id).then(user => {
                        user.send(`Resin capped <t:${bc.resin.recovery_time}:R>.`)
                    }).catch(e => {
                        this.client.sendToLog("Could not DM", `user: ${user_id}`, e);
                    })
                } else {
                    this.client.sendToLog("Resin is triggered again");
                }
            }
        } catch (e) {
...
export default function(client) {
    const genshin_resin = new GenshinResin(client);
    return genshin_resin;
}

...
Pokemon.search = async function(term) {
    if (!isNaN(term)) {
...
Pokemon.getMove = async function(name) {
    let move = json.move.find(entry=>{
        return entry.name == name
    });
    if (!move) return null;
    try {
        let data = fs.readFileSync(`data/pokemon/move/${ability.name}.json`, {encoding: 'utf8'})
...
Pokemon.getAbility = async function(name) {
...
Pokemon.getPokemonByID = async function(id) {
